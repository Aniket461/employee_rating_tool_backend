...
app.post("/sprintrating", async (req, res) => {
  ...
  const existingRating = await SprintRating.find({
  ...
  if (existingRating.length > 0) {
    ...
    ratings.push(rating);
    SprintRating.updateOne(
      { _id: existingRating[0]._id },
      { $set: { ratings: ratings } }
    )
      .then(() => {
  ...
  if (sprintratings.length > 0) {
    ...
    sprintratings.forEach((e) => {
      formattedData.push({
        _id: e._id,
        ratings: e.ratings,
        sprint_start_date: e.sprint_details[0].start_date,
        sprint_end_date: e.sprint_details[0].end_date,
      });
    });
